...
int comDrvInit(int argc, char *argv[])
{
        ...
        for (int i=1; i<argc; ++i)
        {
            ...
            */
            ...
            /* else
            {
                if (myRank==0)
                {
                    std::cout << "rocRhoCentralDrv: Unknown argumnet"
                              << ss.str() << std::endl;
                }
                return -1;
            } */
    ...
    if (runParallel && nProcs > 1)
    {
        if (myRank==0)
        {
            std::cout << "rocRhoCentralDrv: Running in PRALLEL with solver " 
                      << solverType << "." << std::endl;
        }
    }
    ...
    else
    {
        ...
        if (myRank==0)
        {
            std::cout << "rocRhoCentralDrv: Running in SERIAL with solver " 
                      << solverType << "." << std::endl;
        }
    }
...
int comDrvStart(int argc, char *argv[])
{
    ...
    //rocRhoCentral_load_module(winName.c_str(), solverType);
    COM_LOAD_MODULE_STATIC_DYNAMIC(rocrhocentral, winName.c_str());
...
int comDrvStep(const char* name)
{
    ...
    Info << "rocRhoCentralDrv: Stepping status = " 
         << *fluidRun
         << endl;

    Info << "rocRhoCentralDrv: Simulation time = "
         << *fluidTime
         << endl;

    Info << "rocRhoCentralDrv: Simulation Time step = "
         << *fluidDeltaT
         << endl;
...
int comDrvFin(const char* name)
{
    ...
    //COM_UNLOAD_MODULE_STATIC_DYNAMIC(rocRhoCentral, "ROCFOAM");
    //rocRhoCentral_unload_module(name, solverType);
    COM_UNLOAD_MODULE_STATIC_DYNAMIC(rocrhocentral, name);
