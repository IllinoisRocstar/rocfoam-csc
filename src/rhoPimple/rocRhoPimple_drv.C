...
COM_EXTERN_MODULE(rocrhopimple);
...
int comDrvInit(int argc, char *argv[])
{
    ...
    if (myRank==0)
    {
        std::cout << "rocRhoPimpleDrv: Setting up communicator..."
                  << std::endl;

        std::cout << "rocRhoPimpleDrv:Rank " << myRank
                  << ", NProc = " << nProcs
                  << ", COMM = " << masterComm
                  << std::endl;
        ...
        for (int i=1; i<argc; ++i)
        {
            ...
            */
            ...
            /* else
            {
                if (myRank==0)
                {
                    std::cout << "rocRhoPimpleDrv: Unknown argumnet"
                              << ss.str() << std::endl;
                }
                return -1;
            } */
    ...
    if (runParallel && nProcs > 1)
    {
        if (myRank==0)
        {
            std::cout << "rocRhoPimpleDrv: Running in PRALLEL with solver " 
                      << solverType << "." << std::endl;
        }
    }
    else if (!runParallel && nProcs > 1)
    {
        if (myRank==0)
        {
            std::cout << "rocRhoPimpleDrv: NProc>1 detected for a serial job."
                      << std::endl;
            return -1;
        }    
    ...
    else
    {
        ...
        if (myRank==0)
        {
            std::cout << "rocRhoPimpleDrv: Running in SERIAL with solver " 
                      << solverType << "." << std::endl;
        }
    }
...
int comDrvStart(int argc, char *argv[])
{
    ...
    //rocRhoPimple_load_module(winName.c_str(), solverType);
    COM_LOAD_MODULE_STATIC_DYNAMIC(rocrhopimple, winName.c_str());
...
int comDrvStep(const char* name)
{
    ...
    Info << "rocRhoPimpleDrv: Stepping status = " 
         << *fluidRun
         << endl;

    Info << "rocRhoPimpleDrv: Simulation time = "
         << *fluidTime
         << endl;

    Info << "rocRhoPimpleDrv: Simulation Time step = "
         << *fluidDeltaT
         << endl;
...
int comDrvFin(const char* name)
{
    ...
    //COM_UNLOAD_MODULE_STATIC_DYNAMIC(rocRhoPimple, "ROCFOAM");
    //rocRhoPimple_unload_module(name, solverType);
    COM_UNLOAD_MODULE_STATIC_DYNAMIC(rocrhopimple, name);
