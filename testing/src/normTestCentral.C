/*^^^^^^^^^^^^^^ INFINITY NORM TEST ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Testing infinity norm errors of rocRhoCentral.
...
performed for rocRhoCentral in serail mode, but can be extended,
if required.
*/
...
TEST_F(regressionTest, regression)
{
    ...
    char* solverType = const_cast<char *>("rocRhoCentral");
