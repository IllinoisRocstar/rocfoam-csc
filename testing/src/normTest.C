/*^^^^^^^^^^^^^^ INFINITY NORM TEST ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Testing infinity norm errors of rocRhoCentral and rocRhoPimple.
...
performed for rocRhoCentral and rocRhoPimple in seerail mode, but
...
TEST_F(regressionTest, regression)
{
    ...
    {
        regTest = new rhoCentral();
    }
    else if (solverType == const_cast<char *>("rocRhoPimple"))
    {
        regTest = new rhoPimple();
    }
    
    regTest->errorEvaluate(ARGC1, ARGV1);
    EXPECT_LE(regTest->testStat, 1e-10) << "Testing " << solverType <<": regression unsuccessful"
                                        << std::endl;
